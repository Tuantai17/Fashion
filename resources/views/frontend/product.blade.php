                        ...
                        <div class="product-all">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 text-center">
                                @foreach ($products as $product)
                                    @php
                                        ...
                                        if ($product->price_sale > 0 && $product->price_sale < $product->price_root) {
                                            $discountPercent = round(
                                                (($product->price_root - $product->price_sale) / $product->price_root) *
                                                    100,
                                            );
                                    ...
                                    <div
                                        class="border rounded-lg overflow-hidden bg-white shadow hover:shadow-lg transition-all duration-300 hover:scale-105">
                            ...
                            class="w-full h-80 object-cover rounded" alt="{{ $product->name }}"> --}}
                                        ...
                                        <div class="text-center p-4">
                                            ...
                                            <h2
                                                class="text-base rounded-lg relative text-gray-800 my-2 hover:text-[rgb(244,184,198)]">
                                                <a
                                            ...
                                            <div class="flex justify-center items-center gap-2 mb-2">
                                                @if ($product->price_sale > 0 && $product->price_sale < $product->price_root)
                                                    <span
                                                        class="text-red-600 font-bold text-lg">{{ number_format($product->price_sale) }}₫</span>
                                                    <span
                                                        class="line-through text-gray-500 text-sm">{{ number_format($product->price_root) }}₫</span>
                                                @else
                                                    <span
                                                        class="text-gray-800 font-bold text-lg">{{ number_format($product->price_root) }}₫</span>
                                                @endif
                                            </div>
                                            ...
                                            @if ($discountPercent > 0)
                                                <div class="flex justify-center items-center gap-2 mt-1">
                                                    <span class="text-sm text-gray-600">Tiết kiệm:
                                                        <span
                                                            class="text-red-600 font-medium">{{ number_format($savedAmount) }}₫</span>
                                                    </span>
                                                </div>
                                            @endif
                                            ...
                                            <div class="mt-4 flex justify-end space-x-4 px-4 pb-4">
                                                <form action="{{ route('site.cart.add') }}" method="POST">
                                                    ...
                                                    <button type="submit"
                                                        class="flex items-center justify-center gap-2  text-white px-4 py-3 rounded-lg w-full transition duration-300 hover:bg-sky-200">
